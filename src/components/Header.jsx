...
const Header = () => {
  ...
  return (
    <>
      <header className="flex items-center justify-between whitespace-nowrap border-b border-solid border-b-[#3C3D37] bg-[#3C3D37] px-4 md:px-10 py-3 relative">
        <div className="flex items-center gap-4 md:gap-8">
          <Link to="/" className="flex items-center gap-2 md:gap-4 text-white">
            <h2 className="text-white text-lg font-bold leading-tight tracking-[-0.015em]">AURORA</h2>
          </Link>
...
#3C3D37] items-center justify-center pl-4 rounded-l-lg border-r-0">
              ...
              <input
                placeholder="Search"
                className="form-input flex w-full min-w-0 flex-1 resize-none overflow-hidden rounded-lg text-white focus:outline-0 focus:ring-0 border-none bg-[
#3C3D37] focus:border-none h-full placeholder:text-neutral-500 px-4 rounded-l-none border-l-0 pl-2 text-base font-normal leading-normal"
                value={searchQuery}
...
#3C3D37] text-white hover:bg-[#FFDCDC] transition">🛒 Cart</button>
            {user ? (
              <>
                <button onClick={handleLogout} className="px-4 h-10 rounded-lg bg-[#
#3C3D37] text-white text-sm font-semibold hover:bg-[#
#3C3D37] transition">Logout</button>
...
#3C3D37] text-white text-sm font-semibold hover:opacity-90 transition"
              >
...
#3C3D37]">
        ...
        {isMenuOpen && (
          <div className="md:hidden absolute top-full left-0 right-0 bg-white shadow-lg z-50 border-t border-[#3C3D37]">
            <div className="px-4 py-2 space-y-2">
              <div className="flex items-center gap-2 rounded-lg bg-[#FFE9DD] px-3 py-2">
                ...
                <input
                  ...
                  onKeyDown={(e) => e.key === 'Enter' && handleSearchSubmit()}
                />
                <button onClick={handleSearchSubmit} className="text-white hover:text-[#0c77f2]" aria-label="Search">
                  🔍
                </button>
              </div>
<Link 
  ...
  About us ｡°✩
              ...
              <div className="pt-2 border-t border-gray-200">
                {user ? (
                  ...
                  <Link to="/login" onClick={() => setIsMenuOpen(false)} className="block px-4 py-3 text-black hover:bg-[#FFE9DD] rounded-lg transition font-medium">Login</Link>
                )}
              </div>
            </div>
          </div>
